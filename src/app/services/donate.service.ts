...
const baseUrl = environment.SERVER+'/donates';
...
export class DonateService {
  ...
  private allPosts$ = new BehaviorSubject<Donate[]>(undefined);
  ...
  getAll(): Observable<Donate[]> {
    console.log("i m in get all loop");
    return this.http.get<Donate[]>(baseUrl);
  }

  // get(id: any): Observable<Donate> {
  ...
  query(searchstring: any): Observable<Donate[]> {
    // console.log("i m in wild search loop");
    this.http.get<Donate[]>(`${baseUrl}/search?term=${searchstring}`).subscribe(awards=>this.allPosts$.next(awards));
    return this.watchAll().pipe(catchError(this.errorMgmt));
  }

   //to display all awards of particular gym
   watchAll(): Observable<Donate[]> {
