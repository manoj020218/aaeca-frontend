...
const baseUrl = environment.SERVER+'/obits';
...
export class ObituaryService {
  ...
  private allPosts$ = new BehaviorSubject<Obituary[]>(undefined);
  ...
  selectedPost:Obituary;
  awards:Obituary[];

  
  getAll(): Observable<Obituary[]> {
    console.log("i m in get all loop");
    return this.http.get<Obituary[]>(baseUrl);
  }

  // get(id: any): Observable<Obituary> {
  ...
  query(searchstring: any): Observable<Obituary[]> {
    // console.log("i m in wild search loop");
    this.http.get<Obituary[]>(`${baseUrl}/search?term=${searchstring}`).subscribe(awards=>this.allPosts$.next(awards));
    return this.watchAll().pipe(catchError(this.errorMgmt));
  }

   //to display all awards of particular gym
   watchAll(): Observable<Obituary[]> {
