...
const baseUrl = environment.SERVER+'/chapters';
...
export class ChapterService {
  ...
  private allPosts$ = new BehaviorSubject<Chapter[]>(undefined);
  ...
  getAll(): Observable<Chapter[]> {
    ...
    return this.http.get<Chapter[]>(baseUrl);
  }

  // get(id: any): Observable<Chapter> {
  ...
  query(searchstring: any): Observable<Chapter[]> {
    // console.log("i m in wild search loop");
    this.http.get<Chapter[]>(`${baseUrl}/search?term=${searchstring}`).subscribe(awards=>this.allPosts$.next(awards));
    return this.watchAll().pipe(catchError(this.errorMgmt));
  }

   //to display all awards of particular gym
   watchAll(): Observable<Chapter[]> {
