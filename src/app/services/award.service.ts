...
export class AwardService { 
  ...
  private allPosts$ = new BehaviorSubject<Award[]>(undefined);
  ...
  getAll(): Observable<Award[]> {
    ...
    return this.http.get<Award[]>(baseUrl);
  }

  // get(id: any): Observable<Award> {
  ...
  query(searchstring: any): Observable<Award[]> {
    // console.log("i m in wild search loop");
    this.http.get<Award[]>(`${baseUrl}/search?term=${searchstring}`).subscribe(awards=>this.allPosts$.next(awards));
    return this.watchAll().pipe(catchError(this.errorMgmt));
  }

   //to display all awards of particular gym
   watchAll(): Observable<Award[]> {
