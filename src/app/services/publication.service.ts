...
const baseUrl = environment.SERVER+'/publications';
...
export class PublicationService {
  ...
  private allPosts$ = new BehaviorSubject<Publication[]>(undefined);
  ...
  query(searchstring: any): Observable<Publication[]> {
    // console.log("i m in wild search loop");
    this.http.get<Publication[]>(`${baseUrl}/search?term=${searchstring}`).subscribe(awards=>this.allPosts$.next(awards));
    return this.watchAll().pipe(catchError(this.errorMgmt));
  }

   //to display all awards of particular gym
   watchAll(): Observable<Publication[]> {
